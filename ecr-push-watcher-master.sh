#!/bin/bash
. ./master-watcher.config
DOCKER_LOGIN=`aws ecr get-login --no-include-email`
sudo $DOCKER_LOGIN
sudo docker tag itm-watch-transfer:latest $REPOSITORY_FULL_NAME
sudo docker push $REPOSITORY_FULL_NAME
aws ecs register-task-definition --family ${TASK_NAME} --cli-input-json file://task-definition-watcher.json --region ${REGION}
